...
extern std::map<int,FAIL_FILE_DATA_BLOCK> gFailFileDataBlock;

extern int GetOpcodeSize(BYTE opcode,BYTE spcode);

extern void CreateFailFile(char* name);
#include "Utils.h"
void StackWalker::OnDbgHelpErr(LPCSTR szFuncName, DWORD gle, DWORD64 addr)
{
	...
	{
		CHAR buffer[STACKWALK_MAX_NAMELEN];
		_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "[SUCCESS] Address: %p\n",(LPVOID) addr);
		OnOutput(buffer);
		return;
	}

	BYTE opcode=0,spcode=0;

	DWORD naddr = (DWORD)addr;

	DWORD OldProtect;

	VirtualProtect((void*)naddr,sizeof(WORD),PAGE_EXECUTE_READWRITE,&OldProtect);

	opcode = *(BYTE*)(naddr+0);

	spcode = *(BYTE*)(naddr+1);

	VirtualProtect((void*)naddr,sizeof(WORD),OldProtect,&OldProtect);

	FAIL_FILE_DATA_BLOCK block;

	block.off = naddr;

	block.size = GetOpcodeSize(opcode,spcode);

	if(block.size > 0)
	{
		gFailFileDataBlock.insert(std::pair<int,FAIL_FILE_DATA_BLOCK>(block.off,block));

		CreateFailFile("FailFile.txt");

		CHAR buffer[STACKWALK_MAX_NAMELEN];
		_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "[SUCCESS] Address: %p , Opcode: %X , Spcode: %X , Size: %d\n",(LPVOID) addr,opcode,spcode,block.size);
		OnOutput(buffer);
	}
	else
	{
		CHAR buffer[STACKWALK_MAX_NAMELEN];
		_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "[FAIL] Address: %p , Opcode: %X , Spcode: %X , Size: %d\n",(LPVOID) addr,opcode,spcode,block.size);
		OnOutput(buffer);
		TCHAR lString[500];
		_stprintf_s(lString,"UNKNOW OPCODE");
