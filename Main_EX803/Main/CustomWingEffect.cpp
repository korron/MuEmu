...
BOOL CCustomWingEffect::SetWingEffect(int ItemIndex,DWORD a,DWORD b,DWORD c,DWORD d) // OK
{
	...
	for(int n=0;n < MAX_CUSTOM_WING_EFFECT;n++)
	{
		if(this->m_CustomWingEffectInfo[n].ItemIndex == ItemIndex)
		{
			...
			((void(__thiscall*)(void*,DWORD,DWORD,DWORD,DWORD))0x00549D4F)((void*)a,b,c,this->m_CustomWingEffectInfo[n].EffectValue,this->m_CustomWingEffectInfo[n].EffectLevel);

			((void(*)(DWORD,DWORD,float,float*,DWORD,float,DWORD))0x00786D35)(this->m_CustomWingEffectInfo[n].EffectIndex,b,this->m_CustomWingEffectInfo[n].ColorSize,ItemColor,d,this->m_CustomWingEffectInfo[n].ColorMain,(DWORD)this->m_CustomWingEffectInfo[n].ColorSide);
		}
	}
